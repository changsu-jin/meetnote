...
### Phase D: 실시간 회의 어시스턴트 (미래)

**D-1. AI 회의 인사이트**
- 녹음 중 일정 간격(~10분)마다 누적된 전사 텍스트를 Claude에 전달
- vault의 과거 회의록/프로젝트 문서를 RAG로 검색하여 관련 정보 제공
- 인사이트를 회의록 문서에 실시간 삽입

예상 인사이트 유형:
- **과거 회의 연결**: "이 주제는 3/28 스프린트 리뷰에서도 논의됨 → [[2026-03-28 스프린트 리뷰]]"
- **액션아이템 추적**: "이전 회의에서 이민지님이 담당한 API 버그 수정이 아직 미완료"
- **중간 요약**: "지금까지 논의 정리: 1) ... 2) ... 3) ..."
- **용어/컨텍스트 제공**: 새 참석자를 위해 프로젝트 용어 자동 해설

제약:
- Claude Max 구독자 전용 (Claude CLI 사용, 비용 0원 원칙 유지)
- 10분 이상 간격으로 호출 (의미있는 맥락이 쌓인 후)
- 수동 트리거 옵션: "지금까지 분석" 버튼

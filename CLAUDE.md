...
2. **작업 연속성 유지 (세션 독립적)** — 세션이 끊기고 다시 접속해도 이어서 작업 가능해야 함. 사용자가 작업 중단을 선언하면 (예: "여기서 끊자", "오늘은 여기까지") 반드시 `TEST_PROGRESS.md` + backlog + 메모리를 업데이트하여 다음 세션에서 맥락 그대로 이어갈 수 있도록 저장.

...
**1단계: 기능 검증 + 실 회의 테스트**
1. 추가 기능 개별 테스트 (실 녹음 필요 없이 검증 가능한 것부터)
   - [ ] Slack 전송: webhook URL 설정 → 테스트 버튼 → 실제 전송 확인
   - [ ] 화자 매핑: POST /speakers/register로 화자 등록 → 다음 녹음에서 자동 매칭
   - [ ] 암호화: encryption_enabled=true → 녹음 후 .wav.enc 생성 + 원본 삭제 확인
   - [ ] 태그/링크: 회의록 생성 후 frontmatter 태그 + 연관 회의 [[링크]] 확인
   - [ ] 대시보드: "회의 트렌드 대시보드" 명령어 실행 → 통계 생성 확인
   - [ ] 이전 컨텍스트: 두 번째 녹음에서 이전 요약/액션아이템 자동 로드 확인
   - [ ] RAG 검색: "과거 회의 검색" 명령어 → 질의응답 확인
2. **REC-35** 실 대면 회의 테스트 — 화자구분/전사 품질 실제 성능, 전체 파이프라인 E2E

**2단계: 안정화 (실 회의 결과 기반)**
2. **REC-2** 에러 핸들링 — 실 회의에서 발견된 에러 케이스 우선 대응
3. **REC-7** 구조화된 로깅 — 디버깅/모니터링 기반
4. **REC-12** UX 개선 — 에러 메시지, 온보딩, 취소 기능

**3단계: 코드 품질**
5. **REC-3** Backend 단위 테스트 (pytest)
6. **REC-4** Plugin 단위 테스트 (Vitest)
7. **REC-5** 입력 검증 및 Pydantic 스키마
8. **REC-8** 코드 품질 도구 (black, mypy, ESLint)

**4단계: 인프라**
9. **REC-6** CI/CD 파이프라인
10. **REC-9** 보안 강화
11. **REC-10** 통합/E2E 테스트
12. **REC-15** 아키텍처 리팩토링

> 실 회의 테스트(REC-35) 결과에 따라 2~4단계 순서가 조정될 수 있음.

...
if [ -z "$CURRENT_TOKEN" ]; then
    ...
    if [ -n "$HF_TOKEN" ]; then
        sed -i '' "s|huggingface_token: \"\"|huggingface_token: \"$HF_TOKEN\"|" backend/config.yaml
        ok "토큰 설정 완료"
    else
...
if [ -d "$VAULT_PATH" ]; then
    ...
    echo "  cp plugin/main.js plugin/manifest.json <vault>/.obsidian/plugins/meetnote/"
fi
...
echo ""
echo "  사용 방법:"
echo "  1. 서버 시작:  bash start.sh"
echo "  2. Obsidian에서 마크다운 문서 열기"
echo "  3. 마이크 아이콘 클릭 → 녹음 시작"
echo ""

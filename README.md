...
기본 동작에는 환경변수 설정이 필요 없습니다. 필요한 경우 `.env` 파일을 생성하세요.

```bash
# .env (필요한 항목만 추가)
API_KEY=your-secret-key        # 원격 서버 인증
SMTP_USER=your@gmail.com       # 이메일 전송
SMTP_PASSWORD=앱비밀번호        # Gmail 앱 비밀번호
```

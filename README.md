...
- Python 3.10+
- Node.js 18+
- [BlackHole](https://existential.audio/blackhole/) (화상회의 시스템 오디오 캡처용)
- HuggingFace 계정 + 토큰 (무료, [pyannote 이용약관 동의](https://huggingface.co/pyannote/speaker-diarization-3.1))
...
# config.yaml에 HuggingFace 토큰 설정
# diarization.huggingface_token: "hf_..."
...
# Obsidian vault에 복사
cp main.js manifest.json /path/to/vault/.obsidian/plugins/meetnote/
```

### 4. 사용
1. Obsidian에서 마크다운 문서 열기
2. 리본 아이콘(마이크) 클릭 → 녹음 시작
3. 회의 진행 (준실시간 전사 표시)
4. 녹음 중지 → 화자구분 + 요약 + 문서 완성
